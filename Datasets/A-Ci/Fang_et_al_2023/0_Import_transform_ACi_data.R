...
setwd(file.path(path,'/Datasets/A-Ci/Fang_et_al_2023'))
...
curated_data=original_data[,c("SampleID", "SampleID","A..mmol.m2.s.","Ci..ppm.","Ca..ppm.","Ca..ppm.","gs..mol.m2.s.","gs..mol.m2.s.","Iinc..mmol.m2.s.","Tleaf....","Tleaf....","Species","Species_type","Biome","SunShade","Dataset","PFT","Treatment")]

# Rename the columns of the curated dataset with the ESS standard

colnames(curated_data)=c(ESS_column,"Species","Species_type","Biome","SunShade","Dataset","PFT","Treatment")
curated_data$CO2r=NA
curated_data$Record=NA
curated_data$Patm=NA
curated_data$RHs=NA

# I look for the light curves and remove them. They start with a light irradiance at 2000


posi_2000=which(round(curated_data$Qin)==2000)
diff(posi_2000)
AQ_posi=as.vector(sapply(X = posi_2000,FUN = function(x){seq(x,x+9,1)}))
ID_curve=as.vector(sapply(X = posi_2000,FUN = function(x){rep((x-1)/26+1,26)}))
curated_data$SampleID=paste(curated_data$SampleID,ID_curve)
curated_data=curated_data[-AQ_posi,]
curated_data=curated_data[curated_data$Qin>900,]
