...
QC_table=cbind.data.frame(SampleID_num=c(1,17,24,33,62,68,72,73,100,106,212,217,224),
                          Record=c(10,5,5,5,5,5,5,5,10,9,5,5,5)) 
# Here I flag the bad curves by writing down the SampleID_num of the bad curves.
ls_bad_curve=c(66,67,74,103,115,116,164,168,174)

## Some curves have a very low gsw and have no points above Ci = 550 ppm which is too low to estimate Jmax. I remove them
max_Ci_curve=tapply(curated_data$Ci,curated_data$SampleID_num,max)
low_Ci_curve=names(max_Ci_curve[max_Ci_curve<550])
ls_bad_curve=c(ls_bad_curve,low_Ci_curve)
...
Remove_ACi_duplicate<-function(ACi_curve){
  ...
  ACi_curve=round(ACi_curve/2,-1)*2
